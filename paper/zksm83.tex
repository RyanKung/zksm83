...
\begin{equation}
 ...
 \label{eq:msis}
\end{equation}
Nightstream \citep{nightstream} is relevant as an implementation reference for lattice-based
CCS folding architecture, a separated paper-exact versus optimized oracle/sumcheck path, and
seeded Ajtai commitment engineering.  It is not used here as a production backend and is not
evidence of end-to-end SM83 or Game Boy proving throughput.
...
\end{equation}
Before any folding layer is introduced, the implementation types each obligation by family and
evaluation binding: CPU and device row relations use the packed-row sumcheck terminal point; ISA
and execution lookups use shared witness openings; ROM uses a table opening; mutable memory uses
the shared witness plane; continuity uses segment-boundary challenges; and logs use ordered-log
challenges.  Accumulation may only combine claims with the same family and evaluation binding.
...
A benchmark report must separate at least the following buckets before making a performance
claim:
\begin{equation}
\begin{gathered}
\mathsf{WitnessBuild},\mathsf{RelEval},\mathsf{Commit},\mathsf{ROM},
\mathsf{Mem},\mathsf{Cont},\mathsf{Log},\\
\mathsf{Sumcheck},\mathsf{Open},\mathsf{Encode},\mathsf{Verify},|\pi|.
\end{gathered}
\label{eq:benchmark-buckets}
\end{equation}
The current implementation exposes process-local diagnostics for the proof components in
Equation~\eqref{eq:benchmark-buckets}, except that witness construction and proof-byte attribution
still require a dedicated benchmark harness.
...
\mathsf{Bench},\mathsf{Obligation} & \code{optimization}, \code{metrics}, \code{BlockCpuRelation::evaluation\_profile} \\
\bottomrule
\end{tabular}
