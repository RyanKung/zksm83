...
| Receipt memory is bounded across a long segment chain | length-delimited statement/ROM/segment frames, one-frame-at-a-time verifier, seekable prover spool, atomic state/memory progress checkpoint, canonical commitment-identity recovery, and verified four-segment native-only Blue resume |
