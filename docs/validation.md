...
| A versioned native receipt round-trips and verifies without witness or emulator | bounded canonical wire format, typed ROM/memory/log identities, exact expected-statement equality, standalone verifier binary, exhaustive single-byte statement mutation and truncation checks, deterministic receipt noise/hostile-length checks, and real proof/proof-byte/trailing-byte/segment-shape rejection gate |
