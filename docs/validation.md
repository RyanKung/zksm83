...
| Native receipt v2 pair layout is frozen | checked-in `nv14/p128` pair schedule, exact one-precommitment catalog test, 26-to-13 layout invariant, distinct v2 protocol/backend/transcript identity, and bounded two-group PCS gate |
...
| Complete CPU/control semantics | exhaustive positive/negative instruction-family fixtures, machine-step fixtures, 5,355 used slots plus a canonical zero tail in the 6,144-slot row relation, exact used-slot and all-column-read guards, degree-18 algebraic-bound regression, complete native receipt release gate, and one real 16,384-row Blue segment proof |
...
| A versioned native receipt round-trips and verifies without witness or emulator | v2 is the only generation format; explicit v1 read-only decoder/verifier branch; frozen v1 backend digest and statement round-trip; magic/version/statement identity separation; bounded canonical wire format; typed identities; exact expected-statement equality; standalone verifier; mutation, truncation, noise, and hostile-length checks. The expensive end-to-end v2 proof gate remains intentionally unrun |
