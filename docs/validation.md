...
| Receipt memory is bounded across a long segment chain | length-delimited statement/ROM/segment frames, one-frame-at-a-time verifier, seekable prover spool, atomic state/memory progress checkpoint, canonical commitment-identity recovery, and verified two-segment Blue resume |
...
| Adjacent real Blue segments satisfy the complete native proof relation | on the pre-cutover revision, two 16,384-row segments produced and cross-process resume reverified a 37,864,191-byte spool after correcting the degree-18 APU selector composition and canonical memory-identity boundary; the final native-only run restarts from zero |
